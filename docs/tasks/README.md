...
| T06 | `Animal` dumb adapter + physics profile + pooling + reset contract | M2 — adapters | T01 | · | ▫ |
| T07 | `Simulation` tick owner: MoveContext + grace + bounds + collision enqueue→drain→resolve→events + prey-prey impulse | M2 | T02, T03, T06 | · | ▫ |
| T08 | Spawner (UniTask `IAsyncStartable`) + factory + pools + `FieldBounds` (camera frustum) + `GameLifetimeScope` wiring → **vertical slice runs** | M2 | T04, T05, T07 | · | ▫ |
